...
<template>
  <PageHeader title="Inventory List" subtitle="View and manage your current inventory items" />

  {{ allInStock }}
  <div class="flex flex-wrap items-center gap-4 mb-4">
    <InputText v-model="search" placeholder="Search by name" class="w-full sm:w-1/5" />
    ...
    <RouterLink to="/inventory/add" class="ml-auto">
  ...
  >
    ...
    <Column header="Actions" class="text-center">
      <template #body="{ data }">
        <Button icon="pi pi-eye" class="p-button-text p-button-sm mr-2" @click="viewItem(data)" />
        <Button
          icon="pi pi-pencil"
  ...
  <div v-else class="text-gray-500 text-center py-6">No matching inventory items found.</div>
</template>
